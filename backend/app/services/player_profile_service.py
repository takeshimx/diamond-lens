...
def _bat_rank_cols_mart(p: str = "") -> str:
    ...
    return f"""
        ...
        RANK() OVER (ORDER BY {p}so         ASC)  AS so_rank,
        RANK() OVER (ORDER BY {p}woba       DESC) AS woba_rank,
        RANK() OVER (ORDER BY {p}xwoba      DESC) AS xwoba_rank,
        RANK() OVER (ORDER BY {p}xba        DESC) AS xba_rank,
        RANK() OVER (ORDER BY {p}k_pct      ASC)  AS k_pct_rank,
        RANK() OVER (ORDER BY {p}bb_pct     DESC) AS bb_pct_rank,
                ...
                mart_query = f"""
                    ...
                    ranked AS (
                        SELECT
                            ...
                            m.xwoba, m.xba, m.k_pct, m.bb_pct,
