...
WHERE events IS NOT NULL
  AND game_type = "R"
  AND game_year = 2024 -- Dynamic query (Can be all seasons = career level)
  AND batter_name = "Shohei Ohtani" -- Dynamic query
  AND inning >= 7 -- Dynamic query
--   AND strikes = 2 AND balls = 3 -- Dynamic query
  AND home_score = away_score -- Dynamic query
--   AND (away_score - home_score) >= 4 -- Dynamic query
  AND (on_2b != 0 OR on_3b != 0) -- Dynamic query
GROUP BY
  game_year, -- Dynamic query (Can be all seasons = career level)
  batter_name, batter_id
ORDER BY
  game_year ASC, -- Dynamic query (Can be all seasons = career level)
  batter_name ASC